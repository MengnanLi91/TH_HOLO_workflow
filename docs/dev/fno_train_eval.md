...
- Base config: `src/training/config/default.yaml`
- FNO example config: `src/cases/moose_grid/configs/train_fno.yaml`
...
- checkpoint: `output.checkpoint` (default `../data/cases/train_fno/model.mdlus`)
...
| Model | Config | Adapter | Use case |
|-------|--------|---------|----------|
| FNO | `cases/moose_grid/configs/train_fno.yaml` | grid | Regular-grid operator learning |
...
| MLP (FullyConnected) | `cases/alpha_d/configs/train_mlp.yaml` | pointwise | Tabular/axial-profile surrogate |
| Conv1D profile | `cases/alpha_d/configs/train_conv1d.yaml` | profile | 1D convolutional axial-profile surrogate |
