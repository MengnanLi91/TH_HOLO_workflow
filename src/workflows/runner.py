...
class WorkflowRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self.fingerprint_cache_path = self.run_dir / "fingerprint_cache.json"
        self._fingerprint_cache = load_cache(self.fingerprint_cache_path)
        self._refreshed_fingerprint_entries: dict[str, Any] = {}

    def _fingerprint(self, path: Path) -> dict[str, Any]:
        record, refreshed = fingerprint_path(path, cache=self._fingerprint_cache)
        self._fingerprint_cache.update(refreshed)
        self._refreshed_fingerprint_entries.update(refreshed)
        return record

    def _flush_fingerprint_cache(self) -> None:
        if self._refreshed_fingerprint_entries:
            write_cache(self.fingerprint_cache_path, self._fingerprint_cache)
            self._refreshed_fingerprint_entries.clear()

    def _artifact_record(self, artifact: Artifact) -> dict[str, Any]:
        raw_path = Path(artifact.path)
        path = raw_path if raw_path.is_absolute() else self.run_dir / raw_path
        fingerprint = self._fingerprint(path)
        try:
            recorded_path = path.relative_to(self.run_dir).as_posix()
        except ValueError:
    ...
    def _artifacts_valid(self, records: list[dict[str, Any]]) -> bool:
        for record in records:
            raw = Path(record["path"])
            path = raw if raw.is_absolute() else self.run_dir / raw
            if not path.exists():
                return False
    ...
    def _input_fingerprints(self) -> list[dict[str, Any]]:
        ...
        for raw in paths:
            ...
            records.append(self._fingerprint(path))
        return records
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            statuses = [item.get("status") for item in manifest["stages"].values()]
