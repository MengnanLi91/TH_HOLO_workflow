...
def _log_hpo_summary(results: dict[str, Any]) -> None:
    ...
    if n_complete > 0:
        print(
            f"Best trial #{results['best_trial_number']}: val_loss={results['best_value']:.6e}"
        )
...
def evaluate(cfg: dict | Any) -> dict[str, Any]:
    ...
    # Experiment-specific extended metrics (no-op for the base Experiment;
    ...
    if extended_metrics:
        experiment.print_extended_metrics(extended_metrics)
