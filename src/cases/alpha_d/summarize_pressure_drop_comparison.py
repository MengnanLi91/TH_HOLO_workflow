"""Summarize pressure-drop artifacts for coupled versus direct studies.
...
def discover_manifests(study_root: Path) -> list[Manifest]:
    ...
    paths = sorted(study_root.rglob("panel_manifest.json"))
    for path in paths:
        raw = _load_json(path)
        ...
        manifests.append(
            Manifest(
                ...
                moose_verifier_dir=(
                    _resolve_path(raw.get("moose_verifier_dir"), base=base)
